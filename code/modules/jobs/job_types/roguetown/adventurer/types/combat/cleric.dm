...
/datum/outfit/job/roguetown/adventurer/paladin/pre_equip(mob/living/carbon/human/H)
	...
	switch(H.patron?.type)
		if(/datum/patron/old_god)
			...
			var/armors += list("Psydonic Cuirass" = /obj/item/clothing/suit/roguetown/armor/plate/cuirass/fluted/ornate)
		if(/datum/patron/divine/astrata)
	...
	if(H.mind)
		...
		var/oaths = list("Cleric - Health Potion & Training","Crusader - Silver Longsword")
		var/oath_choice = input(H, "Choose your OATH.", "PROFESS YOUR BLESSINGS.") as anything in oaths
		switch(oath_choice)
			if("Cleric - Health Potion & Training")
				H.adjust_skillrank_up_to(/datum/skill/misc/medicine, SKILL_LEVEL_APPRENTICE, TRUE) //Modest improvement, but still inferior to the 'Physician Apprentice' virtue's boons.
				l_hand = /obj/item/needle/thorn //Works in tandem with the health potion to further specialize as a healer. Take the virtue for uncapped skilltraining beyond Apprentice and better supplies.
				beltl = /obj/item/reagent_containers/glass/bottle/rogue/healthpot 
			if("Crusader - Silver Longsword")
				H.adjust_skillrank_up_to(/datum/skill/combat/swords, SKILL_LEVEL_JOURNEYMAN, TRUE)
				l_hand = /obj/item/rogueweapon/sword/long/silver // -5 damage, but the ability to inflict Sunder on undead opponents. Functions similarly to the uncapped Monster Hunter roles of yore.
