...
/obj/item/rogueweapon/blowrod/get_mechanics_examine(mob/user)
    . = ..()
...
/obj/item/rogueweapon/blowrod/proc/attach_heated_glass(obj/item/natural/glass/heated/G, mob/living/user)
	if(!G)
		return FALSE
	if(loaded_glass)
		to_chat(user, span_warning("There is already some heated glass on the blowing rod."))
		return FALSE
...
/obj/item/rogueweapon/blowrod/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/rogueweapon/tongs))
		...
		// If tongs are empty and rod has glass, detach it.
			...
			to_chat(user, span_warning("Either the tongs aren't holding onto some heated glass, or the blowing rod is empty."))
			return TRUE
	return ..()
...
/obj/item/rogueweapon/blowrod/attack_right(mob/living/user)
	...
	var/obj/item/natural/glass/heated/G = loaded_glass
	if(!G)
		to_chat(user, span_warning("I need to attach some heated glass to the blowing rod first."))
		return
	...
	if(skill_level < G.selected_recipe.craftdiff)
		to_chat(user, span_warning("I can't fashion such glassworks without a [SSskills.level_names_plain[G.selected_recipe.craftdiff]]'s skill in pottery."))
	...
	user.visible_message(span_notice("[user] finishes shaping the molten glass into [final_recipe_name]."), span_notice("I finish shaping the glass into [final_recipe_name]."))
