...
/obj/item/proc/pottery_throw_shatter(atom/hit_atom, datum/thrownthing/thrownthing)
	...
	new /obj/item/natural/clay_shard(loc)
...
/obj/item/reagent_containers/glass/bottle/claybottle/examine(mob/user)
	. = ..()
	. += span_info("Clay pottery, unlike its alloyed counterparts, can be painted with a dye brush.")
...
/obj/item/roguestatue/clay/examine(mob/user)
	. = ..()
	. += span_info("Clay pottery, unlike its alloyed counterparts, can be painted in a dyebin.")
...
/obj/item/roguestatue/glass
	...
	smeltresult = null	//No resource return
	sellprice = 55		//Silver is roughly 90 mammon, steel is 40. This sits roughly between. It's high skill to make and a bit of a grind so - worth it since resources to make aren't rare..
	obj_flags = CAN_BE_HIT|UNIQUE_RENAME
...
/obj/item/roguestatue/glass/examine(mob/user)
	. = ..()
	. += span_info("Glassed pottery, unlike its alloyed counterparts, can be painted with a dye brush.")

// -------------------- Porcelain --------------------

/obj/item/natural/clay/porcelain
	name = "raw porcelain piece"
	desc = "A meticulously-prepared piece of porcelain, regal and raw. All that it awaits for now is a furnace's heat, to bring out its elegance."
	icon = 'icons/roguetown/items/cooking.dmi'
	icon_state = "clayporcelaincupraw"
	dropshrink = 1
	obj_flags = UNIQUE_RENAME

/obj/item/natural/clay/porcelain/Initialize()
	. = ..()
	if(cooked_type)
		cooktime = 45 SECONDS
...
/obj/item/carvedgem/porcelain
	name = "porcelain base"
	desc = "A piece of glimmering porcelain, hewn through clay-splattered hands to decorate hovels of higher stature."
	icon = 'icons/roguetown/items/cooking.dmi'
...
/obj/item/reagent_containers/glass/bowl/carved/porcelain
	name = "porcelain bowl"
	desc = "A particularly shining bowl of porcelain."
...
/obj/item/cooking/platter/carved/porcelain
	...
	desc = "A particularly perfect platter of porcelain."
...
/obj/item/reagent_containers/glass/bottle/blown
	...
	desc = "A bottle that has been carefully blown from molten glass."
	obj_flags = CAN_BE_HIT|UNIQUE_RENAME
	sellprice = 4

/obj/item/reagent_containers/glass/bottle/blown/examine(mob/user)
	. = ..()
	. += span_info("Bottles made from blown glass can be given a new color by dipping them into a dye bin.")

/obj/item/reagent_containers/glass/bottle/alchemical/blown
	name = "blown alchemical vial"
	desc = "A small vial that has been carefully blown from molten glass."
	obj_flags = CAN_BE_HIT|UNIQUE_RENAME
	sellprice = 2
