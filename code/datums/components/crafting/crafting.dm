...
/datum/component/personal_crafting/proc/calculate_pottery_quality_score(skill_level)
	// Quality is numeric. 5 is Masterwork, 4 is Fine, 3 is Regular - or Component, 2 is such-and-such, and so on.
	// Gating is simple. Masterworks are restricted to Master-tier pottery, Fineworks are restricted to Journeyman+, and everything below is locked to the regular Skillgate threshold of Apprentice.
	var/roll = rand(1, 100)
	switch(skill_level)
		if(SKILL_LEVEL_NONE)
			return 0
